...
select is((select expires_at-wrap_up_at from public.voice_test_sessions where id='55555555-5555-4555-8555-555555555555'),interval '2 minutes','two-minute goodbye period reserved');
select ok((select wrap_up_at <= started_at+interval '10 minutes' and expires_at <= started_at+interval '12 minutes' from public.voice_test_sessions where id='55555555-5555-4555-8555-555555555555'),'conversation and goodbye have bounded deadlines');
select throws_ok($$update public.voice_test_sessions set expires_at=started_at+interval '13 minutes' where id='55555555-5555-4555-8555-555555555555'$$,'23514',null,'cannot extend beyond maximum goodbye period');
...
select public.checkpoint_live_voice_usage('55555555-5555-4555-8555-555555555555',720000,false);
select public.checkpoint_live_voice_usage('55555555-5555-4555-8555-555555555555',15000,false);
select is((select voice_usage_units from public.voice_test_sessions where id='55555555-5555-4555-8555-555555555555'),720000::bigint,'late usage snapshot cannot reduce observed duration');
select public.checkpoint_live_voice_usage('55555555-5555-4555-8555-555555555555',720000,true);
...
select is((select charged_cents from public.voice_test_sessions where id='55555555-5555-4555-8555-555555555555'),60,'twelve minutes including goodbye costs sixty cents, once');
...
select is((select wrap_up_at from public.voice_test_sessions where id='66666666-6666-4666-8666-666666666666'),(select wrap_up_at from public.voice_test_sessions where id='55555555-5555-4555-8555-555555555555'),'restart retains original wind-down time');
select is((select reserved_cents from public.voice_test_sessions where id='66666666-6666-4666-8666-666666666666'),1940,'prior actual charges remain in the shared ledger');
select public.finish_live_voice_test('66666666-6666-4666-8666-666666666666','confirmed_close_usage_unknown');
select is((select charged_cents from public.voice_test_sessions where id='66666666-6666-4666-8666-666666666666'),1940,'unconfirmed usage is never automatically refunded');
