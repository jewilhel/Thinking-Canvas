...
create function private.guard_voice_tool_execution()
...
begin
  ...
  if task.ai_run_id is distinct from new.run_id or task.status <> 'running'
    ...
    or new.tool_name <> 'create_contextual_comment' then
...
create trigger guard_voice_tool_execution
before insert on public.ai_tool_executions
for each row execute function private.guard_voice_tool_execution();
