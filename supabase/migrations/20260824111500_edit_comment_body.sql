create or replace function public.update_comment_body(
...
begin
  ...
  if actor_role is null or target_comment.author_id <> actor_id then
    raise exception 'Editing this comment is not permitted.' using errcode = '42501';
  end if;
  if target_comment.status <> 'open' then
    raise exception 'Closed comments are read-only.' using errcode = '22023';
  end if;
  if normalized_body is null or char_length(normalized_body) not between 1 and 100000 then
...
revoke all on function public.update_comment_body(uuid, text) from public, anon;
grant execute on function public.update_comment_body(uuid, text) to authenticated;
