...
- Add a deterministic build-time generator that reads only the pinned package's SVG assets and metadata, rejects unexpected elements or attributes, converts supported primitives into normalized vector drawing instructions, and emits a versioned metadata/search manifest plus lazy-loadable per-icon asset chunks.
...
- Expand the current progressive Shapes palette into two levels: **Basic shapes** and **Icons**. Icons open a larger searchable browser without permanently covering the canvas.
- Provide search by name and tag, category tabs, recent choices, a virtualized result grid, clear selected/hover/focus states, and a short visible label or accessible tooltip for every icon.
...
- [ ] Add deterministic catalog generation, validation, metadata, search indexing, compatibility versioning, and generation tests.
...
- [ ] Benchmark catalog generation, metadata payload, one-icon lazy load, 100 visible palette tiles, and 1,000 mixed canvas objects; record results before finalizing the asset strategy.
...
| ---------------------------------------------------------------------------------------------------------------------- | ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------- | --------------------------------------------- |
| Thousands of SVG imports inflate the JavaScript bundle or slow the palette.                                            | High / high         | Generate a small metadata index, virtualize the grid, lazy-load versioned icon chunks, and set measured budgets before Slice 2.                                                                                                             | Engineering                   | Open                                          |
