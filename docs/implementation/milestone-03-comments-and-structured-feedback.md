...
- Add strict Zod schemas and one comment command executor for `comment.create`, `comment.reply`, `comment.respond`, and `comment.status`. Human and AI prompt creation use the same `comment.create` shape, target validation, permission result, persistence path, and audit metadata; only logical actor provenance differs.
...
- Execute comment creation, target creation, optional prompt creation, and actor provenance as one transaction so a partial thread cannot be stored. Reply, response, and status commands are individually atomic.
...
- Keep lifecycle overflow, resolve/dismiss, close, and submit actions visually and semantically distinct. Show author identity, relative time, body, replies, and status in a scan-friendly hierarchy while preserving exact timestamps in accessible detail.
...
- Add transactional functions for root comment plus targets plus optional prompt, reply creation, response upsert, and allowed status transitions. Give each function a fixed search path, strict argument validation, explicit membership checks, and least-privilege grants.
...
- Extend pgTAP coverage for owner, editor, commenter, viewer, non-member, unauthenticated, removed-member, human/AI provenance, target isolation, exact retry, collision, response shape/range, one-response-per-user, status transition, history preservation, and cross-canvas denial cases.
...
- [ ] Produce an immutable Git-backed Netlify deploy preview for that commit and confirm context, secret scan, and non-production data.
...
| Comment bubbles and panels may intercept canvas gestures or obscure contextual controls.                                                                            | Medium / high                   | Use explicit overlay layers, viewport clamping, pointer-event boundaries, edge cases, and desktop/tablet regression screenshots.                       | Engineering                   | Mitigated in 35/35 Chromium regression scenarios.                 |
...
- Invitations or membership-management UI, comment mentions/notifications, attachments, reactions, freeform polls, anonymous responses, per-response discussion, comment export, retention/deletion policy, production launch, cross-browser release matrix, and production observability unless separately promoted in the master ledger.
...
- `pnpm db:reset`: passed from a clean local database through all four migrations and the synthetic seed.
- `pnpm db:test`: the known macOS Docker mount wrapper failure recurred earlier in the implementation; the documented direct PostgreSQL fallback passed all suites: collaboration durability 13/13, comments and structured feedback 25/25, and RLS policy matrix 96/96.
- `pnpm build` with local public Supabase values: passed under installed Next.js 16.3.0.
- focused comment Playwright reruns passed after each fix; the final clean-database full Chromium run passed 35/35 scenarios in 32.9 seconds.
...
- Netlify draft deploy `6a863ad230af3cb206e13869` (`https://6a863ad230af3cb206e13869--thinking-canvas.netlify.app`) built successfully and loaded in Codex's in-app browser.
- The deployed sign-in boundary rendered correctly. Authenticated feature verification is pending action-time product-owner confirmation to transmit the synthetic test credentials through the browser.
- This first draft deploy represents the verified working tree before its exact local commit. A commit-addressable draft will replace it before final hosted evidence is claimed.
