...
| Netlify preview, Supabase, or OpenAI credentials/projects may not yet exist.            | Medium / high       | Inventory configuration before each dependent slice; create only approved non-production resources and keep secrets out of Git/evidence.                                     | Product owner and engineering | Netlify and the non-production Supabase preview project are verified for Slice 2 Auth. OpenAI configuration remains unverified until its owning slice.                                                  |
...
Local Slice 3 verification is complete for the first five checklist items. The additive migration replayed from a clean reset. Direct local PostgreSQL execution passed the existing 96-assertion role matrix plus nine collaboration durability assertions; the Supabase test wrapper remains blocked only by the documented macOS Docker mount limitation. Twenty-four unit tests cover reordered and repeated Yjs updates, load overlap, compaction races, and snapshot equivalence. The four-test Chromium suite adds a real two-browser owner/editor session with simultaneous changes, temporary editor disconnect, durable catch-up, fresh-viewer snapshot reload, private Presence, verified pruning, and idempotent recompaction. Netlify preview deployment and remote two-browser evidence remain required before Slice 3 is complete.
