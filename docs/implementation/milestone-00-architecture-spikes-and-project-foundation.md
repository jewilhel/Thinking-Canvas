...
| GitHub branch protection availability | Product owner | Make the repository public so the existing master-plan requirement can be satisfied on the current GitHub plan.                                                                                                                                                              | Before Slice 1 can complete.         | Resolved 2026-08-10: product owner selected public visibility; GitHub now reports the repository as `PUBLIC`. Branch protection itself remains Slice 1 implementation work. |
...
- [ ] Resolve the GitHub plan/visibility decision, then configure `main` protection with pull-request and required-test rules as required by the master plan.
...
- [ ] Connect Netlify and prove distinct preview and production deployment contexts.
...
- [ ] Commit the initial database migrations, indexes, constraints, RLS policies, and synthetic local seed fixtures.
- [ ] Add role-policy integration tests for owner, editor, commenter, viewer, non-member, and unauthenticated identities.
- [ ] Define versioned Zod schemas for shared objects and server payloads.
- [ ] Implement and test the shared human/AI command boundary, including permission denial, undo metadata, audit metadata, and collaboration updates.
...
| Netlify preview, Supabase, or OpenAI credentials/projects may not yet exist.       | Medium / high       | Inventory configuration before Slice 1; create only approved non-production resources and keep secrets out of Git/evidence.                                                | Product owner and engineering | Unverified.                                                                      |
| The repository was private on a GitHub plan that did not expose branch protection. | Confirmed / high    | Product owner selected public visibility. Configure protected `main` after required CI check names exist and retain the settings evidence.                                 | Product owner and engineering | Visibility blocker resolved 2026-08-10; `main` is not protected yet.             |
...
| 2026-08-10 | GitHub API       | Verify repository visibility and read `main` protection after the product-owner decision | Partial: repository visibility is `PUBLIC`, resolving the plan-availability blocker; the API reports `main` is not protected and no repository rulesets exist.                                                                                      | GitHub CLI readback                   | Milestone 0 protected-`main` build item |
