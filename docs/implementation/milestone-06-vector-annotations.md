...
- [ ] Make visible strokes selectable with pointer, touch, keyboard-accessible contextual actions, marquee/zoom-to-fit bounds, and a stable screen-space hit region.
- [ ] Implement whole-stroke position and geometry editing under the approved transform model; route color and thickness through validated style commands.
- [ ] Replace the last-selected-node transformer with one frame and handles around every drag-selected or Shift-clicked annotation/object; commit multi-item move and resize as one reference-safe history batch.
- [ ] Add backward-compatible `solid`, `dashed`, and `dotted` object/connector stroke patterns and consolidate color, thickness, and supported pattern into one expandable floating Stroke palette.
- [ ] Reuse the Stroke palette's shared color/thickness, positioning, focus, responsive, accessible-name, mixed-value, and batch-application behavior for annotation and object selections; expose only common valid properties.
- [ ] Share the current comment-visibility state with the annotation overlay, update the control copy, and prove the toggle is per-user/per-canvas and non-mutating.
- [ ] Implement one idempotent, history-aware Promote action that keeps the same object and geometry visible while the temporary overlay is hidden.
- [ ] Add focused accessibility, tablet, reduced-motion, clipboard/duplicate, Delete, Undo/Redo, reconnect, and collaboration regressions for edited and promoted strokes.
