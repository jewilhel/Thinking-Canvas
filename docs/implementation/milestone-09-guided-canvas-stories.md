...
- 2026-09-08 — Product-owner hands-on acceptance: after reviewing preview `6aa07cbc22a5ea0d92620cfb`, the product owner reported “All the changes are great” and asked to begin closing Milestone 9. This accepts the current scene, narration, caption, menu, compact-navigation, and panel-dismissal experience; it begins the engineering closeout audit but is not yet the separate closure approval or merge authorization.
...
- Final source commit `bb5b6fc` was pushed and deployed from a clean worktree to [preview `6aa056fa7fccca3af81be1ed`](https://6aa056fa7fccca3af81be1ed--thinking-canvas.netlify.app/app/canvases/26ae67a7-4d71-4a86-beff-304f6a97fc4b). Reopened in Codex's in-app browser, authenticated, selected Stage 1, and confirmed the original three scenes, Saved status, caption controls, global toggle, and “AI narration ready” from the existing cached file. This later documentation-only checkpoint does not change preview runtime code.
...
- `pnpm check` passed 81 files / 408 tests and the production build. The authenticated guided-story E2E passed after the caption changes, including keyboard movement/resize persistence, reduced motion, and Axe. Local fake-AI runs do not verify hosted speech generation.
- Hosted Codex verification on `6aa0710cbee556707e364ef1` (`337454e`): the supplied 66-character caption initially measured 320×82 px with two wrapped lines, versus the old 560×112 default. First pointer movement preserved 320×82. Hover exposed handles without shifting text; mouse-out hid them. The user's existing Stage 1 rectangle remained 279.426×87.418 px. Scene change retained the outgoing inert caption briefly, then replaced it with the new caption. Unit coverage additionally checks opacity completion and exit removal. The temporary fourth scene was soft-deleted; original scenes were preserved.
- This narrow-window smoke pass caught the tool dock covering Previous Scene at 816 px. Increased reserved navigation space from 22 to 27 rem and expanded the 800 px E2E to assert non-overlap plus actual backward navigation. The expanded Axe state also found insufficient contrast in “No comments for this scene”; changed that label to zinc-400. These two focused repairs are part of the replacement preview.
- The expanded authenticated E2E passed 1/1 after both repairs, including Axe. Final runtime commit `6722464` was pushed and deployed from a clean worktree as [preview `6aa07281acc144ff4e03670f`](https://6aa07281acc144ff4e03670f--thinking-canvas.netlify.app/app/canvases/26ae67a7-4d71-4a86-beff-304f6a97fc4b). Codex's 816 px browser confirmed Next selects Stage 1, Previous wraps to Scene 3 without changing the active canvas tool, the outgoing caption becomes inert and disappears, and Next returns to Stage 1. The preview remains open for review; no milestone closure or production deployment is claimed.
...
### Same-framing scene capture repair — 2026-09-08

- User reports Add Scene only works after adjusting the zoom; identical framing must support separate captions without a pan/zoom prerequisite.
