...
- [ ] Extend the shared command boundary for create, patch, move, resize, delete, style, and connector endpoint commands.
- [ ] Implement shape, text, connector, and table creation plus pointer and keyboard selection, movement, resizing, content editing, and deletion.
- [ ] Implement shape anchors, free and attached connector endpoints, attachment preview, detach, and geometry recomputation during target movement and resize.
- [ ] Implement applicable fill, outline, typography, and text-size controls with persistent shared values.
- [ ] Complete the end-to-end object/action matrix for all four essential object types and the connector-attachment geometry suite.
- [ ] Manually construct and retain evidence for representative mind-map, procedure, mood-board, and storyboard arrangements using only the shared toolbar and commands.
...
- No database migration, external deployment, pull request, or master-plan checkbox change occurred in Slice 1.
