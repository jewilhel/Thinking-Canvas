...
| ----------------------------- | ------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- |
| **PD-009 — Offline behavior** | Product owner | **Temporary disconnect recovery only:** a fully loaded canvas may continue accepting edits through a transient connection loss, stores its pending update queue locally, retries automatically, and prevents silent navigation while changes remain unsynced; opening an uncached canvas or marketing deliberate offline work remains unsupported. **Deliberate offline editing:** additionally define offline entry, durable local availability, multi-session merge expectations, storage limits, and recovery UX, materially expanding this milestone. | Decided before the reconnect and unsynced-change slice began. | Approved on 2026-08-11: temporary disconnect recovery only. Deliberate offline editing remains excluded. |
...
- Under the approved `PD-009` choice, retain pending updates in a per-user/per-canvas browser store until acknowledged. Warn before navigation or sign-out when unsynced changes remain. Do not allow a new uncached canvas session to start without an authorized server load.
...
- [x] Obtain the product-owner `PD-009` decision and update this plan and the master ledger before implementing its selected behavior.
- [x] Add and test the idempotent-update migration, regenerate database types, and verify local and remote non-production policy behavior.
- [x] Integrate product commands with durable append, overlap-safe load, private Broadcast, snapshots, and retry-safe sequence acknowledgments.
- [x] Add Presence participant/selection state and throttled Broadcast cursors without persistent-history writes.
- [x] Add saved, saving, reconnecting, unsynced, retrying, and failed UI states plus the approved local pending queue and navigation/sign-out protection.
- [x] Add the preview/test-only deterministic simulated AI command driver.
- [x] Automate concurrent two-human and simulated-AI edits, ambiguous append retry, disconnect edits, reconnect, cursor/selection ephemerality, authorization loss, and reload convergence.

### Slice 5 — Milestone verification and evidence

- [x] Run formatting, lint, strict types, unit, migration/RLS, production build, complete Chromium end-to-end, and axe checks on the final branch head.
- [x] Run the protected GitHub `quality` check and retain the exact run and commit references.
...
| ----------------------------------------------------------------------------------------------------- | ------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------- | --------------------------------------------------------------- |
| `PD-009` could expand temporary recovery into a full offline product.                                 | Medium / high       | Keep deliberate offline editing excluded; revise the master ledger and milestone scope before adding offline entry or uncached-canvas support.                              | Product owner                 | Resolved for Milestone 1: temporary disconnect recovery only.   |
...
- Added client-generated update IDs and an idempotent append overload. Repeating the same actor/canvas/update ID returns its original sequence, while reusing the ID for different bytes fails. The legacy append overload remains available for the architecture-spike harness.
- Applied migration `20260811130000_idempotent_canvas_updates.sql` to local Supabase and the linked `thinking-canvas-preview` project, regenerated database types, and confirmed the local and remote migration ledgers match.
- Connected the product document to the approved subscribe-before-load repository, private Broadcast, snapshots, durable sequence acknowledgments, and a one-second durable reconciliation fallback that closes missed or out-of-order Broadcast gaps without moving the contiguous sequence boundary past a missing update.
- Added a per-user/per-canvas validated local pending queue with bounded exponential retry, saved/saving/reconnecting/unsynced/retrying/failed states, manual retry, and browser plus in-app navigation/sign-out warnings while changes remain pending. An already loaded canvas accepts edits during a transient disconnect; deliberate offline entry remains unsupported.
- Added Presence participants and selected-object IDs plus cursor Broadcast capped at eight sends per second. Cursor state is dropped when Presence expires and never enters Yjs, snapshots, the pending queue, or undo history.
- Added a stable preview/test-only AI actor that issues an ordinary validated create command through the same document and durability path. The control is available locally and in Netlify preview/branch-deploy contexts, not production.
- Fixed a connection-lifecycle race discovered by the concurrent browser scenario: a superseded asynchronous connect could clear the active repository after the replacement had subscribed. Connection generations now ensure only the owning attempt may publish or clear repository state.
- Hosted verification found that Netlify did not expose its `CONTEXT` or server-only `APP_ENV` value to the server component at runtime. The preview driver now uses a branch/deploy-preview `NEXT_PUBLIC_APP_ENV` build flag, with production explicitly set to `production`; the driver appeared only after redeploying commit `2b1f92a`.
...
| 2026-08-11 | Local repository                               | `pnpm check` after the Slice 4 durability and recovery implementation                                                                             | Pass: Prettier, ESLint with zero warnings, strict TypeScript, 66 Vitest tests across 20 files, and the Next.js 16.3 production build completed.                                                                                                                                                                                                                                                                                                                                                   | Local command output                                                  | Slice 4 queue validation, collaboration client, build, and regressions             |
| 2026-08-11 | Local Supabase after clean reset               | Direct PostgreSQL execution of the RLS and collaboration pgTAP suites                                                                             | Pass: all 96 policy-matrix assertions and all 13 collaboration-durability assertions passed, including idempotent ambiguous retry, changed-content rejection, immediate append denial after editor membership removal, and private Broadcast/Presence policy presence. The standard wrapper repeated the documented macOS mount failure.                                                                                                                                                          | Direct local PostgreSQL output                                        | Slice 4 idempotency, authorization loss, RLS, Broadcast, and Presence              |
| 2026-08-11 | Supabase `thinking-canvas-preview`             | `supabase db push --include-all` followed by `supabase migration list`                                                                            | Pass: migration `20260811130000` applied to the linked non-production project; local and remote ledgers report all three migrations at matching versions.                                                                                                                                                                                                                                                                                                                                         | Supabase CLI output                                                   | Slice 4 remote non-production schema readiness                                     |
| 2026-08-11 | Local Supabase Auth and Chromium               | `pnpm test:e2e` after a clean reset                                                                                                               | Pass: all 21 scenarios completed. Product coverage includes local pending work through a temporary disconnect, blocked silent navigation, automatic retry, two-human concurrent work, Presence and cursor Broadcast, simulated-AI mutation, reconnect, durable catch-up, and matching object counts after both browsers reload.                                                                                                                                                                   | Playwright output                                                     | Slice 4 two-human/AI collaboration, transient disconnect, cursors, and convergence |
| 2026-08-11 | Authenticated Netlify branch deploy and Chrome | Deploy `6a7bd01ba1b30369ab8c03c1` for commit `2b1f92a`; create a preview recovery canvas, add simulated AI idea, and reload                       | Pass: authenticated owner created canvas `c2f2b6e2-3802-4365-809d-fbfae6efd7a2`; the preview-only simulated AI control created one purple `Simulated AI idea`, reached `Saved` at durable sequence 2, and remained listed/rendered after reload. Screenshot retained in the hosted-browser evidence for this task.                                                                                                                                                                                | Netlify deploy details, signed-in Chrome DOM readback, and screenshot | Slice 4 hosted durability, preview-only driver, and reload persistence             |
| 2026-08-11 | Immutable Netlify deploy preview and Chrome    | Deploy `6a7be88eb980d84dd0ad0729` built from clean local head `dd30ab8`; owner create, simulated-AI edit, reload, sign out/in, and reopen         | Pass: the protected `deploy-preview` context reached `ready`; owner created canvas `f68d27a5-dab6-4d7c-b39a-ce828639a728`, the simulated-AI rectangle persisted at durable sequence 2, and the canvas returned with one object and `Saved` after both reload and sign-in recovery. Netlify reports `commit_ref: null` for this manual deploy and exposes no secret-scan result, so it is retained as hosted behavior evidence but does not yet close the immutable-preview/secret-scan criterion. | Netlify deploy API readback and signed-in Chrome DOM readback         | FR-001 hosted persistence; Slice 4 preview driver and reload recovery              |
| 2026-08-11 | GitHub Actions protected CI                    | [Run `31562387782`](https://github.com/jewilhel/Thinking-Canvas/actions/runs/31562387782) for PR #6 at `dd30ab8d647ee68b6bbdc5e9e68148521c1deca1` | Pass: `quality` job `94007222179` completed in 4m56s. Formatting, lint, strict types, unit tests, clean-start database/RLS checks, build, and 21 Chromium end-to-end/accessibility scenarios all passed.                                                                                                                                                                                                                                                                                          | GitHub Actions run and job readback                                   | Slice 5 protected CI and final automated quality evidence                          |
...
| 2026-08-11 | Selected temporary disconnect recovery only for `PD-009`.                                                          | The product owner explicitly chose bounded recovery from transient connection loss and deferred deliberate offline editing.                  | The master ledger records the approved product boundary; Slice 4 may implement a local pending queue, automatic retry, explicit unsynced states, and navigation/sign-out protection without offline entry.                                                                   | Product owner |
| 2026-08-11 | Completed the local Slice 4 implementation and applied its migration to the non-production preview database.       | The approved disconnect boundary was required before durable product collaboration could replace browser-only persistence.                   | Product commands now persist and converge for two humans and the simulated AI through transient disconnects; local quality, policy, idempotency, authorization-loss, and 21-scenario Chromium gates pass. Hosted branch-deploy verification remains next.                    | Engineering   |
...
| 2026-08-11 | Ran final automated and hosted one-owner recovery verification; opened PR #6 for protected CI.                     | The product owner approved non-production credential submission and a draft PR solely to complete the remaining verification gates.          | CI run `31562387782` passed for `dd30ab8`. Immutable deploy `6a7be88eb980d84dd0ad0729` proved create, AI edit, reload, sign-out/sign-in, and reopen. Hosted two-human convergence, same-preview arrangement evidence, and a platform secret-scan/commit binding remain open. | Engineering   |

## Closure

Closure status: Not ready — hosted two-human convergence, same-preview arrangement evidence, and immutable preview secret-scan/commit binding remain unverified.
