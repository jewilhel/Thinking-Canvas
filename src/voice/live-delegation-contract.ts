...
Only completedTasks and current applied canvas state verify an action. A spoken assistant claim such as “saved” does not prove a document exists. If the participant directly requested a document and there is no confirmed task for that request, fulfill it even when the assistant already claimed success. If they then ask where that missing document is, verify the canvas and complete the still-unfulfilled request rather than repeating the false claim. A polite acknowledgment such as “thank you” after the request does not cancel it. Do not repeat a change that completedTasks or the canvas state actually confirms.
