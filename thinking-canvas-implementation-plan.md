...
- [ ] **FR-052 — Internal collaboration.** Text ranges and internal objects support comments, replies, structured prompts, annotations, and AI-assisted review. The internal-object portion is superseded by `PD-022`; document text-range collaboration remains active. Its comment action appears only in the expanded-text selection palette, opens the established contextual comment experience, retains the chosen range while focus moves into the composer, and leaves an unambiguous collaborator-visible range highlight until the thread is resolved.
