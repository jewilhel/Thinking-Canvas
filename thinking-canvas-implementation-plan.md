...
Pending design revision — 2026-09-11: natural spoken question/comment acceptance failed despite phrase-filter repairs. Proposed: one bounded, tool-free interpretation call using the existing backend model and volatile conversation context before the existing authorized canvas workflow. Preserve client delegation, no saved audio/general transcript, current role/tool restrictions, and the existing shared spending/time limits; account for interpretation even when no action results. This adds a model step to the approved migration and awaits owner approval. Detailed scope and verification: [Context-aware spoken intent repair](docs/implementation/milestone-10-live-conversation.md#context-aware-spoken-intent-repair--proposed-awaiting-approval-2026-09-11). No completion status changes.
