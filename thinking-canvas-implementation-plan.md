...
- [x] **FR-044 — Add document.** A participant can add a general-purpose document container to the canvas.
- [x] **FR-045 — Page-like board representation.** A document is recognizable as a page-like object at canvas level, retains a compact visible page shadow when closed or focused, and has an approved long-content preview behavior.
- [x] **FR-046 — Focused document interaction.** Zooming into a document frames it, enters document interaction, and provides a clear return to the parent canvas.
- [ ] **FR-047 — Rich text.** Documents support bold, italics, underline, and hierarchical outline formatting with keyboard and toolbar access. **Superseded for the first version by `PD-021` and the Markdown-aligned formatting contract in `FR-091`; retained verbatim for source traceability.**
- [x] **FR-048 — AI document editing.** Subject to permission, the AI can generate, edit, and format document content through validated commands.
- [x] **FR-049 — Scrolling layout.** A document can use a responsive continuous scrolling layout.
...
- [ ] **FR-051 — Internal visual objects.** Shapes, text, connectors, tables, and annotations can be placed alongside document text. Superseded for the current product by `PD-022`; canvas objects remain on the parent canvas and do not nest in documents.
- [x] **FR-052 — Internal collaboration.** Text ranges and internal objects support comments, replies, structured prompts, annotations, and AI-assisted review. The internal-object portion is superseded by `PD-022`; document text-range collaboration remains active. Its comment action appears only in the expanded-text selection palette and opens the same contextual composer and thread presentation used by canvas comments, including `@` recipient selection and primary-AI routing, replies, structured prompts, lifecycle actions, retry/cancel behavior, and conversational AI undo. Every authorized document-range thread appears in the shared Comments history panel. The chosen range remains stable while focus moves into the composer, retains an unambiguous collaborator-visible highlight while the thread is open, and reopens that same thread when its highlighted text is tapped or clicked. Resolving, dismissing, or deleting the thread removes its range highlight.
- [ ] **FR-053 — Boundary containment.** Internal objects remain clipped and owned by the document and cannot connect to parent-canvas objects. Superseded by `PD-022`; documents no longer create an internal canvas-object boundary.
- [x] **FR-088 — Markdown interoperability.** A participant can copy selected document content or a complete document as Markdown, paste Markdown as structured rich content, and import or export a Markdown file. Conversion is deterministic, preserves supported text structure and content in both directions, and warns before any Lexical-only or Thinking Canvas-only presentation would be omitted rather than silently discarding it. `PD-023` supersedes the confirmation requirement: export downloads immediately, while document display settings remain intentionally excluded by the established conversion contract.
- [ ] **FR-089 — Modifier-drag document containment.** A participant can Command-drag on macOS or Control-drag on Windows/Linux a supported top-level canvas object or compatible group into a document, see an unambiguous containment preview, and drag it back to the parent canvas without a visual jump or data loss. Ordinary dragging over the same document only overlaps it. Accessible **Place inside document** and **Remove from document** actions provide equivalent non-pointer paths; containment preserves applicable styling, comments, history, collaboration, reconnect, and reload behavior while preventing connections across the document boundary. Superseded by `PD-022`; modifier-drag and accessible containment actions no longer target documents.
- [ ] **FR-090 — Document appearance and layout properties.** In document settings, a participant can change the document's background color, choose one document-wide display font and reading size, and select its layout from continuous scrolling or an approved paginated page size and orientation. The selected properties persist, collaborate, reload, and remain consistent between the page-like canvas representation and focused document interaction. Display font and reading size do not alter document semantics and are ignored during Markdown copy, paste, import, and export.
- [x] **FR-091 — Range-level document text styling.** Within a document body, a participant can select character or paragraph ranges and apply Markdown-aligned semantic formatting: bold, italics, hierarchical headings or outline levels, ordered and unordered lists, and safe links. Invoking link from an expanded selection opens a dark floating URL palette beside that selection; applying or dismissing the palette preserves the selected range, and a linked range has the conventional underline treatment. Inserting a table from a selected text block places the table after that block so the selected text can remain its title. Formatting affects only the intended selection or active paragraph, supports mixed-format selections and keyboard access, and persists through collaboration, reload, Markdown conversion, undo, and redo. Milestone 8 does not expose range-level typeface, font-size, color, alignment, or underline controls beyond the visual affordance of a semantic link.

### Exit gate

- [ ] Complete the sourced **Document collaboration** acceptance scenario and product-owner-added **Document interoperability and composition** scenario in both scrolling and paginated layouts, then verify Markdown round-trip behavior, range-level styling, modifier-drag containment and removal, reload, and concurrent editing. Under `PD-022`, the active exit gate replaces document containment/removal with proof that overlap and modifier-drag keep shapes and groups on the parent canvas and that no document-containment action or embedded-object layer is exposed.

Evidence: the [Milestone 8 first-class documents plan](docs/implementation/milestone-08-first-class-documents.md) records hosted product review and acceptance, real-provider AI repair verification, Markdown and document UX checks, and local regression results. [PR #13](https://github.com/jewilhel/Thinking-Canvas/pull/13) merged as `e61b238` after [quality run 34172103740](https://github.com/jewilhel/Thinking-Canvas/actions/runs/34172103740) passed on reviewed head `a8c5219`, including database/RLS, units, build, and browser/accessibility tests. The product owner approved closure on 2026-09-07. Checked active requirements below reflect these combined records, not merge alone.

Ledger review: `FR-047`, `FR-051`, `FR-053`, and `FR-089` remain unchecked because their original behavior was superseded, not delivered. `FR-050` and the full `FR-090` remain unchecked because selecting paginated layouts is temporarily deferred; background, font, reading size, and continuous presentation are delivered. The milestone exit gate remains unchecked: the recorded exact-head hosted role/reconnect, responsive/accessibility, and performance matrix has not been completed. Product acceptance and merge are complete; formal engineering closure and production verification are not claimed.
...
Approved conversational ending — 2026-09-15: ten minutes starts a natural wind-down, followed by at most two additional minutes for closing thoughts and salutations. The supervisor requests a warm transition at minute ten, firmer encouragement to continue in a new session during the final minute, and a brief final goodbye before the hard disconnect at minute twelve. Conversation instructions may customize greeting and goodbye style, not the deadline. Reserve and meter the entire possible duration under the existing $20 daily allowance. This narrow goodbye period is approved within current Netlify hosting; the deferred 30-minute/Render effort remains out of scope. Existing running sessions keep their previously issued deadline.


Deferred duration extension and Render hosting — 2026-09-15: owner deferred the earlier 30-minute session request and the proposed move of the voice supervisor to Render. Retain the 10-minute limit and existing Netlify supervisor while completing the existing milestones before further experience refinements. The $20 daily allowance, pre-expiry warning, explicit new-session action, and save-on-request transcript policy remain unchanged. Longer sessions and Render hosting are future work, not Milestone 10 implementation or closure requirements. Preserve the proposal, rationale, and future acceptance checks in the [milestone plan](docs/implementation/milestone-10-live-conversation.md#deferred-thirty-minute-sessions-and-render-hosting). Revisit only when the product owner brings this effort back into scope; no new hosting or migration is authorized.

Approved activity presentation and recovery — 2026-09-14: automatic voice task records belong only in Comments history, grouped by session; only explicitly requested comments create canvas markers. Preserve existing action records while removing their automatic markers. Renew expired preview sign-in separately without reloading the canvas or losing unsaved conversation text. This does not enable automatic transcript storage.

Approved full-session source correction — 2026-09-14: requested transcripts, summaries and briefs must use the full captured voice-session wording, not only the recent command context. Exact transcripts are copied from that source into a new or explicitly targeted existing document. Retain the source temporarily and save only on request under the existing privacy decision; automatic durable session history is a separate pending preference. Disclose source gaps and reject oversized source rather than silently presenting a partial result as complete.

Approved session-boundary correction — 2026-09-14: keep temporary conversations individually identified by session and start time, default transcript viewing/saving to the latest session, and allow selecting earlier in-tab sessions. Pass the most recent prior captured conversation separately to Canvas AI so requested previous-session transcripts and synthesis do not combine sessions. No automatic durable transcript history. Unexpected endings must explain the reason; retain the approved pre-limit warning and explicit restart.

Approved completion scope — 2026-09-14: requested transcripts must assemble streaming fragments into readable speaker turns. Add Canvas AI/voice selection and named-document open/close navigation for the requesting participant, verify document editing and existing grouping/ungrouping and one-level parent/child commands end to end. Larger generated structures/templates remain Milestone 11. The completion slices are implemented on the milestone branch: requested transcript assembly, participant-local navigation, document body editing, and undoable grouping/containment. Browser and database evidence is recorded in the Milestone 10 execution log; final owner voice acceptance and milestone closure remain pending.

Approved capability revision — 2026-09-14: Voice exposes the same Canvas AI actions available for the current canvas/document/scene scope, with conversation saving and spoken undo added. Remove the voice-only action subset and single-action rejection. Voice visual heuristics are advisory, not vetoes or automatic placement/style overrides. Canonical command validation, membership authorization, durable persistence, cancellation and undo remain. This supersedes the incremental voice action rollout; implementation and acceptance evidence are tracked in the milestone plan.

Approved design revision — 2026-09-11: send bounded recent conversation wording directly to the existing Canvas AI on a provider delegation. The owner chose this over the proposed separate interpretation model call. Remove phrase matching as the admission gate; the same existing backend call understands the request and answers or uses permitted tools. Preserve client delegation, volatile general conversation, current authorization and cancellation protections, and existing spending/time limits. No additional model or model call. Detailed amendment: [Milestone 10](docs/implementation/milestone-10-live-conversation.md). The owner accepted natural spoken canvas questions and contextual-comment creation on 2026-09-11; broader migration and milestone acceptance remain pending.

Approved scope revision — 2026-09-08 (`PD-001`): deliver AI-only live conversation now. Remote-human voice (`FR-010`) and its transport implementation/testing are explicitly deferred to reduce cost and complexity. Their original wording remains below for traceability and their boxes remain unchecked. Milestone 10 closure covers the remaining active requirements, supporting work, and unchanged Live co-thinking exit gate; it does not claim human-voice delivery. Detailed plan: [Milestone 10 — Live conversation](docs/implementation/milestone-10-live-conversation.md). The owner subsequently approved the documented implementation slices and Netlify background supervisor, retained the current controls/defaults, and settled the application-announcement pause policy for this milestone. Timing verification and milestone acceptance remain pending.

Approved architecture feasibility test — 2026-09-09: hosted pre-peer sideband attachment returns HTTP 404. The owner approved the milestone record's D6 feasibility test to start the deadline supervisor first, complete a muted peer handshake, and require sideband attachment within 10 seconds before normal audio/controls. This introduces the explicitly approved bounded pre-attachment interval; implementation and hosted verification are authorized. The original 10-minute session cap remains unchanged; the owner subsequently raised the shared daily allowance to $20 on 2026-09-09; no FR or exit gate is accepted by this proposal.
...
- [ ] All `FR-001` through `FR-091` items are checked with linked evidence.
...
- [ ] **AS-008 — Document interoperability and composition.** A participant creates a document, changes its background, document-wide display font, reading size, and layout, formats selected text ranges, pastes and imports representative Markdown, and copies and exports the result back to Markdown with any unsupported presentation disclosed. The Markdown result contains no display-font or reading-size presentation data. The participant modifier-drags supported canvas objects into the document and back out, confirms ordinary overlap does not create containment, and verifies that document-owned text, visuals, comments, connections, history, collaboration, and reload remain bounded to the document in both continuous and paginated layouts. Under `PD-022` and `PD-023`, the active scenario instead proves immediate export, normalized import titles, editable imported titles, and that overlapping or modifier-dragged shapes/groups remain independent parent-canvas objects with no document containment UI.
...
- [x] **PD-005 — Important interruption:** define testable examples and non-examples for `FR-013` and `FR-014`. **Decision, 2026-09-25:** application-triggered canvas-task reports and observations do not interrupt active participant speech in Milestone 10. Deliver still-relevant results once at a natural pause; discard stale or superseded results. A blocking failure stops its action and appears visually immediately, while its spoken explanation waits for a pause. Native voice turn-taking and participant barge-in remain separate. An explicit “tell me immediately” spoken-update option is desirable but deferred. Examples: a completed requested edit or document save waits and is reported once; a permission failure blocks the action immediately and is spoken later. Non-examples for interruption: routine success, suggestions, praise, disagreement, uncertainty, duplicate results, stale results, quoted commands, and silence alone. `FR-013`/`FR-014` remain unchecked until conversation evaluation confirms this behavior.
  ...
  - 2026-09-08: Product owner approved a temporary button-opened live conversation control panel to experiment with the then-applicable Realtime API settings. The GPT-Live migration superseded that API inventory. On 2026-09-25 the owner chose to retain the implemented single-button layout, secondary settings panel, and current baseline for this milestone. The pause policy and its examples are settled above; `FR-013` and `FR-014` remain unchecked pending conversation evaluation.
...
- [ ] **PD-007 — Performance budgets:** approve target hardware, representative canvas size, latency thresholds, and maximum acceptable degradation.
  ...
  - 2026-09-09: Product owner subsequently raised the shared daily voice-testing cap to $20. Preserve existing charges and reservations; keep the 10-minute session cap and midnight-Pacific reset.
...
- [x] **PD-019 — Markdown conversion contract:** use a bounded GitHub Flavored Markdown subset for headings, paragraphs, bold, italics, ordered and unordered lists, safe links, and tables; reject raw executable HTML. Export semantic document text and tables only after a preflight warning identifies internal spatial objects, comments, annotations, and display settings that Markdown cannot represent. Document-wide display font, reading size, background, layout, page size, and orientation are neither inferred from nor serialized into Markdown. Approved by the product owner with the Milestone 8 plan on 2026-09-02. The preflight-warning portion is superseded by `PD-023`; the semantic subset and presentation exclusions remain active.
- [x] **PD-020 — Document containment matrix:** permit shapes, text, icons, tables, annotations, and complete compatible groups. Permit a free connector or a connector whose attached endpoints move in the same atomic family; reject any connector that would cross the document boundary. Preserve annotation temporary or durable state and do not allow documents inside documents. Approved by the product owner with the Milestone 8 plan on 2026-09-02. Superseded in full by `PD-022` after hosted testing showed that embedded canvas objects rendered inconsistently and conflicted with Markdown portability.
