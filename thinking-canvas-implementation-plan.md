...
- [x] **PD-024 — Viewport-captured scene workflow:** create a scene by capturing the canvas's current position and zoom through an **Add Scene** action; present scenes as an ordered, draggable list; allow rename, replacement from the current view, and deletion; and provide previous/next scene controls whose canvas transitions feel exceptionally smooth. Place the scene-editor control between previous and next; use one selected-scene earlier/later control below a visually simplified list; and make bidirectional end wrapping an enabled-by-default, persistent per-user/per-canvas **Loop** preference. Do not add scene printing or PDF export. Initial workflow approved by the product owner on 2026-09-07 using three supplied interaction-reference screenshots; motion accepted and control/list/loop refinement approved through hosted-preview review on 2026-09-07.
